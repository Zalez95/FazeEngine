...
namespace se::app {
	...
	public:		// Functions
		...
		virtual void execute() override
		{
			...
			SetOperation(Operation::DepthTest).bind();
	...
	public:		// Functions
		...
		virtual void execute() override
		{
			...
			GraphicsOperations::setOperation(Operation::DepthTest, false);
		};
	};
		...
		BindableRenderNode(name), mMergeShadowsNode(nullptr), mShadowUniformsUpdater(nullptr)
	{
		...
		auto iDepthTextureBindable = resources->addBindable();
		resources->addOutput( std::make_unique<BindableRNodeOutput<Texture>>("depthTexture", resources, iDepthTextureBindable) );

		if (mergeShadowsNode->findInput("target")->connect( resources->findOutput("shadowTarget") )
			&& mergeShadowsNode->findInput("depthTexture")->connect( resources->findOutput("depthTexture") )
			...
			mDepthTextureBindableIndex = addBindable();
			addInput(std::make_unique<BindableRNodeInput<Texture>>("depthTexture", this, mDepthTextureBindableIndex));
		}
	}
	...
	void ShadowRenderSubGraph::setBindable(std::size_t bindableIndex, const BindableSPtr& bindable)
	{
		...
		else if (bindableIndex == mDepthTextureBindableIndex) {
			auto resources = dynamic_cast<BindableRenderNode*>(mGraph.getNode("resources"));
			auto output = dynamic_cast<BindableRNodeOutput<Texture>*>( resources->findOutput("depthTexture") );
			resources->setBindable(output->getBindableIndex(), bindable);
		}
